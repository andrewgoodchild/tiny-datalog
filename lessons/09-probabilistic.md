# Lesson 9 — Probabilistic Datalog, honestly

> **Follows on from lesson 8.** Assumes you have met semirings.
...
why-provenance, from Lesson 8. That is exactly how real systems do it:
...
sets, not from a semiring. `exercises/08-homomorphism.py`'s companion
does exactly that — enumerate the 2⁵ worlds, keep the ones where s
reaches t, sum their probabilities:

```
$ python3 exercises/09-exact-prob.py
...
sets once (lesson 8), then hand them to a weighted model counter. That
...
Next: [incremental maintenance](10-incremental.md) — repairing answers
