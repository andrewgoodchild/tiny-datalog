# Lesson 10 — Incremental maintenance: don't recompute the world
...
Note the connection to Lesson 8: DRed's phase 1 over-approximates "facts
...
`--explain` tree shows you *a* route; Lesson 8's why-provenance is the
...
- **Wall-clock goes the other way here.** On this engine B/F is
  *slower* (about 1.5s to DRed's 0.85s on the run above) even though it
  ...
  magic sets paid in Lesson 7. The number B/F optimises is facts
...
- **The backward search must be well-founded.** A path fact around a
  ...
  Lesson 8's diverging count semiring. Counting-based maintenance
...
One name ties this lesson to Lesson 8: Val Tannen is both the Tannen of
...
algebra of changes needs subtraction, exactly the operation Lesson 8's
...
Next: [Horn clauses](11-horn-clauses.md). The boundary the whole
language lives on.
